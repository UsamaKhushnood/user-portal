    ...
    <div class="tab-content">
      <div class="tab-pane show active">
        <div
          ...
          v-for="(job, projectIndex) in jobsList"
          :key="projectIndex"
          v-b-modal="'view-cancelled-job' + projectIndex"
        >
          <div class="task-content ticket-prgs-common">
            ...
            <div class="tckt-t-w">
              ...
              <span class="c-red">{{ job.status }}</span>
            </div>
          </div>
          <div class="stm-abt-tckt c-red">
            <span>Ticket ID: {{ job.ticketId }}</span>
            <span>Ticket Reason: {{ job.ticketReason }}</span>
            <span>Canceled by: {{ job.claimedBy }}</span>
          </div>
          <ViewCancelledJobModal :job="projectIndex"></ViewCancelledJobModal>
        </div>
      </div>
    </div>
  </div>
...
<script>
import ViewCancelledJobModal from "./modals/in-cancel-modals/ViewCancelledJobModal";
export default {
  components: { ViewCancelledJobModal },
  data() {
    return {
      jobsList: [
        {
