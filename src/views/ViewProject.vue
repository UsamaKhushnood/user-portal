<template>
  <div id="view-project">
    <div class="container tab-content-parent">
      <div class="row">
        <div class="col-xl-3 col-lg-3 col-md-12 col-sm-12 col-12">
          <div class="ui-block">
            <div class="your-profile">
              <div
                class="rt-panel-h inv-rt-panel-h crsr-pntr"
                data-toggle="modal"
                data-target="#invent-freelancer"
              >
                <div class="inv-flc">
                  <span>
                    <svg class="">
                      <use xlink:href="#warning"></use>
                    </svg>
                  </span>
                  <div class="inv-det">
                    <span class="inv-det-h">Invent Freelancer</span>
                    <span class="inv-det-txt">Get more quote</span>
                  </div>
                </div>
              </div>
            </div>
          </div>
          <div class="container-parent-tab ui-block">
            <div class="row">
              <div class="col col-12">
                <div class="your-profile">
                  <div class="rt-panel-h">Categories</div>
                </div>
                <ul class="nav nav-tabs nav-justified">
                  <li class="nav-item">
                    <a class="nav-link active">Project Proposals</a>
                    <ul class="sub-menu">
                      <li class="active">
                        <span class="c-red">15</span
                        ><a
                          href="http://ihannoveraner.de/service-user/view-proposals.php"
                          >New Proposals</a
                        >
                      </li>
                      <li>
                        <span class="c-red">2</span><a href="#">Remember</a>
                      </li>
                      <li>
                        <span class="c-red">1</span><a href="#">Rejected</a>
                      </li>
                      <li>
                        <span class="c-red">4</span><a href="#">Messaged</a>
                      </li>
                      <li>
                        <span class="c-red">2</span><a href="#">Reported</a>
                      </li>
                    </ul>
                  </li>
                  <li class="nav-item">
                    <a
                      class="nav-link"
                      href="http://ihannoveraner.de/service-user/project-description.php"
                      >Project Description</a
                    >
                  </li>
                  <li class="nav-item">
                    <a class="nav-link">Project Tasks</a>
                  </li>
                  <li class="nav-item">
                    <a class="nav-link">Project Files</a>
                  </li>
                  <li class="nav-item">
                    <a class="nav-link">Project Requiemnts</a>
                  </li>
                  <li class="nav-item">
                    <a class="nav-link">Project Budget</a>
                  </li>
                </ul>
              </div>
            </div>
          </div>
          <div class="ui-block">
            <div class="your-profile">
              <div class="rt-panel-h">Overview</div>
              <div class="ststc ststc-left">
                <span class="ststc-h">Post Date and Time</span>
                <span>12.10.2012 – 19:30</span>
              </div>
              <div class="ststc ststc-left">
                <span class="ststc-h">Total View</span>
                <span>400 views</span>
              </div>
              <div class="ststc ststc-left">
                <span class="ststc-h">Project expired in</span>
                <span>5 days , 10 Hours , 30 min</span>
              </div>
            </div>
          </div>
        </div>
        <div
          class="
            col-xl-9 col-lg-9 col-md-12 col-sm-12 col-12
            tab-content tab-content-nbg
          "
        >
          <div class="row t-container tab-pane show active" id="tab-2-1">
            <div class="col-12 tab-sec-ticket tab-not">
              <div class="filter-common">
                <div class="filter-inner-comon"><span>Filter</span></div>
                <div class="filter-inner-comon">
                  <input type="text" placeholder="Search..." />
                </div>
                <div class="filter-inner-comon">
                  <select>
                    <option>Sort by</option>
                    <option>Sort 1</option>
                    <option>Sort 2</option>
                    <option>Sort 3</option>
                  </select>
                  <div class="srt-dd-active">
                    <svg
                      version="1.1"
                      id="Capa_1"
                      xmlns="http://www.w3.org/2000/svg"
                      xmlns:xlink="http://www.w3.org/1999/xlink"
                      x="0px"
                      y="0px"
                      width="255px"
                      height="255px"
                      viewBox="0 0 255 255"
                      style="enable-background: new 0 0 255 255"
                      xml:space="preserve"
                    >
                      <g>
                        <g id="arrow-drop-down">
                          <polygon
                            points="0,63.75 127.5,191.25 255,63.75   "
                          ></polygon>
                        </g>
                      </g>
                      <g></g>
                      <g></g>
                      <g></g>
                      <g></g>
                      <g></g>
                      <g></g>
                      <g></g>
                      <g></g>
                      <g></g>
                      <g></g>
                      <g></g>
                      <g></g>
                      <g></g>
                      <g></g>
                      <g></g>
                    </svg>
                  </div>
                </div>
              </div>
              <div class="row">
                <div class="col-xl-9 col-lg-12 col-md-12 col-sm-12 col-12">
                  <div
                    class="pane-common-inner pop-over crsr-pntr"
                    data-toggle="modal"
                    data-target="#smart-system"
                  >
                    <div class="task-content-row">
                      <div class="no-prop">
                        <div class="no-prop-img">
                          <img src="@/assets/img/no-proposal.png" />
                        </div>
                        <span class="no-prop-y">No Proposals yet</span>
                        <span class="no-prop-inv"
                          >Invent Freelancer to your Project</span
                        >
                      </div>
                    </div>
                  </div>
                  <div class="pane-common-inner">
                    <div class="task-content-row">
                      <div class="row-prop-container">
                        <div class="row-prop-tags row-prop-std">Recommand</div>
                        <div class="row-prop-tags row-prop-nhre c-red">
                          New here
                        </div>
                        <div class="row-prop-inner-col row-prop-img">
                          <img
                            src="@/assets/img/mencap.png"
                            class="no-radius"
                          />
                        </div>
                        <div
                          class="row-prop-inner-col row-prop-content-container"
                        >
                          <div class="row-prop-c-name">
                            <span>Company Name</span>
                          </div>
                          <div class="row-prop-c-det">
                            <div class="row-prop-inner-col row-prop-content">
                              <svg class="olymp-close-icon">
                                <use xlink:href="#rocket-ship"></use>
                              </svg>
                              <span class="row-prop-h">Work Start</span>
                              <span class="row-prop-txt">Now</span>
                            </div>
                            <div class="row-prop-inner-col row-prop-content">
                              <svg class="olymp-close-icon">
                                <use xlink:href="#calendar-1"></use>
                              </svg>
                              <span class="row-prop-h">Work Type</span>
                              <span class="row-prop-txt"
                                >Fix – Hourly - Week</span
                              >
                            </div>
                            <div class="row-prop-inner-col row-prop-content">
                              <svg class="olymp-close-icon">
                                <use xlink:href="#calendar-1"></use>
                              </svg>
                              <span class="row-prop-h">Warranty</span>
                              <span class="row-prop-txt">Yes – 3 Weeks</span>
                            </div>
                            <div class="row-prop-inner-col row-prop-content">
                              <span class="row-prop-h row-prop-h-g"
                                >250,00 €</span
                              >
                              <span class="row-prop-txt"
                                >Hourly – 10 € hr.</span
                              >
                            </div>
                          </div>
                        </div>
                      </div>
                      <div class="row-prop-detail">
                        <div class="row-prop-dl-col">
                          <svg class="row-prop-dl-icon">
                            <use xlink:href="#worldwide"></use>
                          </svg>
                          <span class="row-prop-dl-txt">GE</span>
                        </div>
                        <div class="row-prop-dl-col">
                          <svg class="row-prop-dl-icon">
                            <use xlink:href="#location-1"></use>
                          </svg>
                          <span class="row-prop-dl-txt">Hannover</span>
                        </div>
                        <div class="row-prop-dl-col">
                          <svg class="row-prop-dl-icon">
                            <use xlink:href="#bubble-speak"></use>
                          </svg>
                          <span class="row-prop-dl-txt">0</span>
                        </div>
                        <div class="row-prop-dl-col">
                          <svg class="row-prop-dl-icon">
                            <use xlink:href="#clock-1"></use>
                          </svg>
                          <span class="row-prop-dl-txt">0 Hours</span>
                        </div>
                        <div class="row-prop-dl-col">
                          <svg class="row-prop-dl-icon">
                            <use xlink:href="#verify"></use>
                          </svg>
                          <span class="row-prop-dl-txt">0 Jobs</span>
                        </div>
                      </div>
                    </div>
                  </div>
                  <div class="pane-common-inner">
                    <div class="task-content-row">
                      <div class="row-prop-container">
                        <div class="row-prop-tags row-prop-sctd">Selected</div>
                        <div class="row-prop-inner-col row-prop-img">
                          <img
                            src="@/assets/img/fl-user.png"
                            class="no-radius"
                          />
                        </div>
                        <div
                          class="row-prop-inner-col row-prop-content-container"
                        >
                          <div class="row-prop-c-name">
                            <span>Freelancer Name</span>
                            <div class="row-prop-rating-container">
                              <svg class="row-prop-rating-svg">
                                <use xlink:href="#star-2"></use>
                              </svg>
                              <svg class="row-prop-rating-svg">
                                <use xlink:href="#star-2"></use>
                              </svg>
                              <svg class="row-prop-rating-svg">
                                <use xlink:href="#star-2"></use>
                              </svg>
                              <svg class="row-prop-rating-svg">
                                <use xlink:href="#star-2"></use>
                              </svg>
                              <svg class="row-prop-rating-svg">
                                <use xlink:href="#star-2"></use>
                              </svg>
                            </div>
                          </div>

                          <div class="row-prop-c-det">
                            <div class="row-prop-inner-col row-prop-content">
                              <svg class="olymp-close-icon">
                                <use xlink:href="#rocket-ship"></use>
                              </svg>
                              <span class="row-prop-h">Work Start</span>
                              <span class="row-prop-txt">Now</span>
                            </div>
                            <div class="row-prop-inner-col row-prop-content">
                              <svg class="olymp-close-icon">
                                <use xlink:href="#calendar-1"></use>
                              </svg>
                              <span class="row-prop-h">Work Type</span>
                              <span class="row-prop-txt"
                                >Fix – Hourly - Week</span
                              >
                            </div>
                            <div class="row-prop-inner-col row-prop-content">
                              <svg class="olymp-close-icon">
                                <use xlink:href="#calendar-1"></use>
                              </svg>
                              <span class="row-prop-h">Warranty</span>
                              <span class="row-prop-txt">Yes – 3 Weeks</span>
                            </div>
                            <div class="row-prop-inner-col row-prop-content">
                              <span class="row-prop-h row-prop-h-g"
                                >250,00 €</span
                              >
                              <span class="row-prop-txt"
                                >Hourly – 10 € hr.</span
                              >
                            </div>
                          </div>
                        </div>
                      </div>
                      <div class="row-prop-detail">
                        <div class="row-prop-dl-col">
                          <svg class="row-prop-dl-icon">
                            <use xlink:href="#worldwide"></use>
                          </svg>
                          <span class="row-prop-dl-txt">GE</span>
                        </div>
                        <div class="row-prop-dl-col">
                          <svg class="row-prop-dl-icon">
                            <use xlink:href="#location-1"></use>
                          </svg>
                          <span class="row-prop-dl-txt">Hannover</span>
                        </div>
                        <div class="row-prop-dl-col">
                          <svg class="row-prop-dl-icon">
                            <use xlink:href="#bubble-speak"></use>
                          </svg>
                          <span class="row-prop-dl-txt">0</span>
                        </div>
                        <div class="row-prop-dl-col">
                          <svg class="row-prop-dl-icon">
                            <use xlink:href="#clock-1"></use>
                          </svg>
                          <span class="row-prop-dl-txt">0 Hours</span>
                        </div>
                        <div class="row-prop-dl-col">
                          <svg class="row-prop-dl-icon">
                            <use xlink:href="#verify"></use>
                          </svg>
                          <span class="row-prop-dl-txt">0 Jobs</span>
                        </div>
                      </div>
                    </div>
                  </div>
                  <div class="pane-common-inner">
                    <div class="task-content-row">
                      <div class="row-prop-container">
                        <div class="row-prop-tags row-prop-rcd">Recommand</div>
                        <div class="row-prop-inner-col row-prop-img">
                          <img
                            src="@/assets/img/mencap.png"
                            class="no-radius"
                          />
                        </div>
                        <div
                          class="row-prop-inner-col row-prop-content-container"
                        >
                          <div class="row-prop-c-name">
                            <span>Company Name</span>
                            <div class="row-prop-rating-container">
                              <svg class="row-prop-rating-svg">
                                <use xlink:href="#star-2"></use>
                              </svg>
                              <svg class="row-prop-rating-svg">
                                <use xlink:href="#star-2"></use>
                              </svg>
                              <svg class="row-prop-rating-svg">
                                <use xlink:href="#star-2"></use>
                              </svg>
                              <svg class="row-prop-rating-svg">
                                <use xlink:href="#star-2"></use>
                              </svg>
                              <svg class="row-prop-rating-svg">
                                <use xlink:href="#star-2"></use>
                              </svg>
                            </div>
                          </div>
                          <div class="row-prop-c-det">
                            <div class="row-prop-inner-col row-prop-content">
                              <svg class="olymp-close-icon">
                                <use xlink:href="#rocket-ship"></use>
                              </svg>
                              <span class="row-prop-h">Work Start</span>
                              <span class="row-prop-txt">Now</span>
                            </div>
                            <div class="row-prop-inner-col row-prop-content">
                              <svg class="olymp-close-icon">
                                <use xlink:href="#calendar-1"></use>
                              </svg>
                              <span class="row-prop-h">Work Type</span>
                              <span class="row-prop-txt"
                                >Fix – Hourly - Week</span
                              >
                            </div>
                            <div class="row-prop-inner-col row-prop-content">
                              <svg class="olymp-close-icon">
                                <use xlink:href="#calendar-1"></use>
                              </svg>
                              <span class="row-prop-h">Warranty</span>
                              <span class="row-prop-txt">Yes – 3 Weeks</span>
                            </div>
                            <div class="row-prop-inner-col row-prop-content">
                              <span class="row-prop-h row-prop-h-g"
                                >250,00 €</span
                              >
                              <span class="row-prop-txt"
                                >Hourly – 10 € hr.</span
                              >
                            </div>
                          </div>
                        </div>
                      </div>
                      <div class="row-prop-detail">
                        <div class="row-prop-dl-col">
                          <svg class="row-prop-dl-icon">
                            <use xlink:href="#worldwide"></use>
                          </svg>
                          <span class="row-prop-dl-txt">GE</span>
                        </div>
                        <div class="row-prop-dl-col">
                          <svg class="row-prop-dl-icon">
                            <use xlink:href="#location-1"></use>
                          </svg>
                          <span class="row-prop-dl-txt">Hannover</span>
                        </div>
                        <div class="row-prop-dl-col">
                          <svg class="row-prop-dl-icon">
                            <use xlink:href="#bubble-speak"></use>
                          </svg>
                          <span class="row-prop-dl-txt">0</span>
                        </div>
                        <div class="row-prop-dl-col">
                          <svg class="row-prop-dl-icon">
                            <use xlink:href="#clock-1"></use>
                          </svg>
                          <span class="row-prop-dl-txt">0 Hours</span>
                        </div>
                        <div class="row-prop-dl-col">
                          <svg class="row-prop-dl-icon">
                            <use xlink:href="#verify"></use>
                          </svg>
                          <span class="row-prop-dl-txt">0 Jobs</span>
                        </div>
                      </div>
                    </div>
                  </div>
                  <div class="pane-common-inner">
                    <div class="task-content-row">
                      <div class="row-prop-container">
                        <div class="row-prop-tags row-prop-ptr">Partner</div>
                        <div class="row-prop-inner-col row-prop-img">
                          <img
                            src="@/assets/img/fl-user.png"
                            class="no-radius"
                          />
                        </div>
                        <div
                          class="row-prop-inner-col row-prop-content-container"
                        >
                          <div class="row-prop-c-name">
                            <span>Freelancer Name</span>
                            <div class="row-prop-rating-container">
                              <svg class="row-prop-rating-svg">
                                <use xlink:href="#star-2"></use>
                              </svg>
                              <svg class="row-prop-rating-svg">
                                <use xlink:href="#star-2"></use>
                              </svg>
                              <svg class="row-prop-rating-svg">
                                <use xlink:href="#star-2"></use>
                              </svg>
                              <svg class="row-prop-rating-svg">
                                <use xlink:href="#star-2"></use>
                              </svg>
                              <svg class="row-prop-rating-svg">
                                <use xlink:href="#star-2"></use>
                              </svg>
                            </div>
                          </div>

                          <div class="row-prop-c-det">
                            <div class="row-prop-inner-col row-prop-content">
                              <svg class="olymp-close-icon">
                                <use xlink:href="#rocket-ship"></use>
                              </svg>
                              <span class="row-prop-h">Work Start</span>
                              <span class="row-prop-txt">Now</span>
                            </div>
                            <div class="row-prop-inner-col row-prop-content">
                              <svg class="olymp-close-icon">
                                <use xlink:href="#calendar-1"></use>
                              </svg>
                              <span class="row-prop-h">Work Type</span>
                              <span class="row-prop-txt"
                                >Fix – Hourly - Week</span
                              >
                            </div>
                            <div class="row-prop-inner-col row-prop-content">
                              <svg class="olymp-close-icon">
                                <use xlink:href="#calendar-1"></use>
                              </svg>
                              <span class="row-prop-h">Warranty</span>
                              <span class="row-prop-txt">Yes – 3 Weeks</span>
                            </div>
                            <div class="row-prop-inner-col row-prop-content">
                              <span class="row-prop-h row-prop-h-g"
                                >250,00 €</span
                              >
                              <span class="row-prop-txt"
                                >Hourly – 10 € hr.</span
                              >
                            </div>
                          </div>
                        </div>
                      </div>
                      <div class="row-prop-detail">
                        <div class="row-prop-dl-col">
                          <svg class="row-prop-dl-icon">
                            <use xlink:href="#worldwide"></use>
                          </svg>
                          <span class="row-prop-dl-txt">GE</span>
                        </div>
                        <div class="row-prop-dl-col">
                          <svg class="row-prop-dl-icon">
                            <use xlink:href="#location-1"></use>
                          </svg>
                          <span class="row-prop-dl-txt">Hannover</span>
                        </div>
                        <div class="row-prop-dl-col">
                          <svg class="row-prop-dl-icon">
                            <use xlink:href="#bubble-speak"></use>
                          </svg>
                          <span class="row-prop-dl-txt">0</span>
                        </div>
                        <div class="row-prop-dl-col">
                          <svg class="row-prop-dl-icon">
                            <use xlink:href="#clock-1"></use>
                          </svg>
                          <span class="row-prop-dl-txt">0 Hours</span>
                        </div>
                        <div class="row-prop-dl-col">
                          <svg class="row-prop-dl-icon">
                            <use xlink:href="#verify"></use>
                          </svg>
                          <span class="row-prop-dl-txt">0 Jobs</span>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
                <div
                  class="
                    col-xl-3 col-lg-12 col-md-12 col-sm-12 col-12 col-props
                  "
                >
                  <div class="ui-block">
                    <div class="ls-dets">
                      <img class="ls-brs-img" src="@/assets/img/v-prop-1.png" />
                      <span class="ls-s-dets-com ls-dets-bp"
                        >View all Proposals</span
                      >
                    </div>
                  </div>
                  <div class="ui-block">
                    <div class="ls-dets">
                      <img class="ls-brs-img" src="@/assets/img/v-prop-2.png" />
                      <span class="ls-s-dets-com ls-dets-bp"
                        >Clear all requiements</span
                      >
                    </div>
                  </div>
                  <div class="ui-block">
                    <div class="ls-dets">
                      <img class="ls-brs-img" src="@/assets/img/v-prop-3.png" />
                      <span class="ls-s-dets-com ls-dets-bp"
                        >Select the right Company</span
                      >
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
          <div class="row t-container tab-pane" id="tab-2-2">
            <div class="col-12 tab-sec-ticket tab-not">
              <div class="filter-common">
                <div class="filter-inner-comon"><span>Filter</span></div>
                <div class="filter-inner-comon">
                  <input type="text" placeholder="Search..." />
                </div>
                <div class="filter-inner-comon">
                  <select>
                    <option>Sort by</option>
                    <option>Sort 1</option>
                    <option>Sort 2</option>
                    <option>Sort 3</option>
                  </select>
                  <div class="srt-dd-active">
                    <svg
                      version="1.1"
                      id="Capa_1"
                      xmlns="http://www.w3.org/2000/svg"
                      xmlns:xlink="http://www.w3.org/1999/xlink"
                      x="0px"
                      y="0px"
                      width="255px"
                      height="255px"
                      viewBox="0 0 255 255"
                      style="enable-background: new 0 0 255 255"
                      xml:space="preserve"
                    >
                      <g>
                        <g id="arrow-drop-down">
                          <polygon
                            points="0,63.75 127.5,191.25 255,63.75   "
                          ></polygon>
                        </g>
                      </g>
                      <g></g>
                      <g></g>
                      <g></g>
                      <g></g>
                      <g></g>
                      <g></g>
                      <g></g>
                      <g></g>
                      <g></g>
                      <g></g>
                      <g></g>
                      <g></g>
                      <g></g>
                      <g></g>
                      <g></g>
                    </svg>
                  </div>
                </div>
              </div>
              <div class="task-content ticket-prgs-common">
                <div class="prgs-avt">
                  <img src="@/assets/img/t-image.png" /><img
                    class="prgs-avtflg"
                    src="data:image/png;base64,"
                    alt=""
                  />
                </div>
                <div class="tckt-t-w">
                  <span><b>Jobs Title</b></span>
                  <span>Project ID</span>
                </div>
                <div class="tckt-t-w">
                  <span><b>Tasks</b></span>
                  <span class="c-green"><font class="c-red">4</font> / 3</span>
                </div>
                <div class="tckt-t-w">
                  <span><b>Milestone</b></span>
                  <span class="c-green"><font class="c-red">3</font> / 2</span>
                </div>
                <div class="tckt-t-w">
                  <span><b>Status</b></span>
                  <span class="c-green">Open</span>
                </div>
              </div>
              <div class="tab-content">
                <div class="tab-pane show active">
                  <div class="task-content-row">
                    <div class="task-content ticket-prgs-common">
                      <div class="prgs-avt">
                        <img src="@/assets/img/t-image.png" /><img
                          class="prgs-avtflg"
                          src="data:image/png;base64,"
                          alt=""
                        />
                      </div>
                      <div class="tckt-t-w">
                        <span><b>Jobs Title</b></span>
                        <span>Project ID</span>
                      </div>
                      <div class="tckt-t-w">
                        <span><b>Tasks</b></span>
                        <span class="c-green"
                          ><font class="c-red">4</font> / 3</span
                        >
                      </div>
                      <div class="tckt-t-w">
                        <span><b>Milestone</b></span>
                        <span class="c-green"
                          ><font class="c-red">3</font> / 2</span
                        >
                      </div>
                      <div class="tckt-t-w">
                        <span><b>Status</b></span>
                        <span class="c-orange">In Hold</span>
                      </div>
                    </div>
                    <div class="stm-abt-tckt c-red">
                      <span>Ticket ID: 1234556</span>
                      <span>Ticket Reason: Work not completed</span>
                      <span>Claimed by: Company</span>
                    </div>
                  </div>
                  <div class="task-content-row">
                    <div class="task-content ticket-prgs-common">
                      <div class="prgs-avt">
                        <img src="@/assets/img/t-image.png" /><img
                          class="prgs-avtflg"
                          src="data:image/png;base64,"
                          alt=""
                        />
                      </div>
                      <div class="tckt-t-w">
                        <span><b>Jobs Title</b></span>
                        <span>Project ID</span>
                      </div>
                      <div class="tckt-t-w">
                        <span><b>Tasks</b></span>
                        <span class="c-green"
                          ><font class="c-red">4</font> / 3</span
                        >
                      </div>
                      <div class="tckt-t-w">
                        <span><b>Milestone</b></span>
                        <span class="c-green"
                          ><font class="c-red">3</font> / 2</span
                        >
                      </div>
                      <div class="tckt-t-w">
                        <span><b>Status</b></span>
                        <span class="c-red">Canceled</span>
                      </div>
                    </div>
                    <div class="stm-abt-tckt c-red">
                      <span>Ticket ID: 1234556</span>
                      <span>Ticket Reason: Work not completed</span>
                      <span>Claimed by: User</span>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
          <div class="row t-container tab-pane" id="tab-2-3">
            <div class="col-12 tab-sec-ticket tab-not">
              <div class="filter-common">
                <div class="filter-inner-comon"><span>Filter</span></div>
                <div class="filter-inner-comon">
                  <input type="text" placeholder="Search..." />
                </div>
                <div class="filter-inner-comon">
                  <select>
                    <option>Sort by</option>
                    <option>Sort 1</option>
                    <option>Sort 2</option>
                    <option>Sort 3</option>
                  </select>
                  <div class="srt-dd-active">
                    <svg
                      version="1.1"
                      id="Capa_1"
                      xmlns="http://www.w3.org/2000/svg"
                      xmlns:xlink="http://www.w3.org/1999/xlink"
                      x="0px"
                      y="0px"
                      width="255px"
                      height="255px"
                      viewBox="0 0 255 255"
                      style="enable-background: new 0 0 255 255"
                      xml:space="preserve"
                    >
                      <g>
                        <g id="arrow-drop-down">
                          <polygon
                            points="0,63.75 127.5,191.25 255,63.75   "
                          ></polygon>
                        </g>
                      </g>
                      <g></g>
                      <g></g>
                      <g></g>
                      <g></g>
                      <g></g>
                      <g></g>
                      <g></g>
                      <g></g>
                      <g></g>
                      <g></g>
                      <g></g>
                      <g></g>
                      <g></g>
                      <g></g>
                      <g></g>
                    </svg>
                  </div>
                </div>
              </div>
              <div class="task-content ticket-prgs-common">
                <div class="prgs-avt">
                  <img src="@/assets/img/t-image.png" /><img
                    class="prgs-avtflg"
                    src="data:image/png;base64,"
                    alt=""
                  />
                </div>
                <div class="tckt-t-w">
                  <span><b>Jobs Title</b></span>
                  <span>Project ID : Category – SubCatgeory</span>
                </div>
                <div class="tckt-t-w">
                  <span><b>Tasks</b></span>
                  <span class="c-green"><font class="c-red">4</font> / 3</span>
                </div>
                <div class="tckt-t-w">
                  <span><b>Milestone</b></span>
                  <span class="c-green"><font class="c-red">3</font> / 2</span>
                </div>
              </div>
              <div class="task-content ticket-prgs-common">
                <div class="prgs-avt">
                  <img src="@/assets/img/t-image.png" /><img
                    class="prgs-avtflg"
                    src="data:image/png;base64,"
                    alt=""
                  />
                </div>
                <div class="tckt-t-w">
                  <span><b>Jobs Title</b></span>
                  <span>Project ID : Category – SubCatgeory</span>
                </div>
                <div class="tckt-t-w">
                  <span><b>Tasks</b></span>
                  <span class="c-green"><font class="c-red">4</font> / 3</span>
                </div>
                <div class="tckt-t-w">
                  <span><b>Milestone</b></span>
                  <span class="c-green"><font class="c-red">3</font> / 2</span>
                </div>
              </div>
              <div class="task-content ticket-prgs-common">
                <div class="prgs-avt">
                  <img src="@/assets/img/t-image.png" /><img
                    class="prgs-avtflg"
                    src="data:image/png;base64,"
                    alt=""
                  />
                </div>
                <div class="tckt-t-w">
                  <span><b>Jobs Title</b></span>
                  <span>Project ID : Category – SubCatgeory</span>
                </div>
                <div class="tckt-t-w">
                  <span><b>Tasks</b></span>
                  <span class="c-green"><font class="c-red">4</font> / 3</span>
                </div>
                <div class="tckt-t-w">
                  <span><b>Milestone</b></span>
                  <span class="c-green"><font class="c-red">3</font> / 2</span>
                </div>
              </div>
            </div>
          </div>
          <div class="row t-container tab-pane" id="tab-2-4">
            <div class="col-12 tab-sec-ticket tab-not">
              <div class="filter-common">
                <div class="filter-inner-comon"><span>Filter</span></div>
                <div class="filter-inner-comon">
                  <input type="text" placeholder="Search..." />
                </div>
                <div class="filter-inner-comon">
                  <select>
                    <option>Sort by</option>
                    <option>Sort 1</option>
                    <option>Sort 2</option>
                    <option>Sort 3</option>
                  </select>
                  <div class="srt-dd-active">
                    <svg
                      version="1.1"
                      id="Capa_1"
                      xmlns="http://www.w3.org/2000/svg"
                      xmlns:xlink="http://www.w3.org/1999/xlink"
                      x="0px"
                      y="0px"
                      width="255px"
                      height="255px"
                      viewBox="0 0 255 255"
                      style="enable-background: new 0 0 255 255"
                      xml:space="preserve"
                    >
                      <g>
                        <g id="arrow-drop-down">
                          <polygon
                            points="0,63.75 127.5,191.25 255,63.75   "
                          ></polygon>
                        </g>
                      </g>
                      <g></g>
                      <g></g>
                      <g></g>
                      <g></g>
                      <g></g>
                      <g></g>
                      <g></g>
                      <g></g>
                      <g></g>
                      <g></g>
                      <g></g>
                      <g></g>
                      <g></g>
                      <g></g>
                      <g></g>
                    </svg>
                  </div>
                </div>
              </div>
              <div class="task-content ticket-prgs-common">
                <div class="prgs-avt">
                  <img src="@/assets/img/t-image.png" /><img
                    class="prgs-avtflg"
                    src="data:image/png;base64,"
                    alt=""
                  />
                </div>
                <div class="tckt-t-w">
                  <span><b>Jobs Title</b></span>
                  <span>Project ID</span>
                </div>
                <div class="tckt-t-w">
                  <span><b>Tasks</b></span>
                  <span class="c-green"><font class="c-red">4</font> / 3</span>
                </div>
                <div class="tckt-t-w">
                  <span><b>Milestone</b></span>
                  <span class="c-green"><font class="c-red">3</font> / 2</span>
                </div>
                <div class="tckt-t-w tckt-t-w-btn">
                  <button class="btn btn-lg btn-green">Close</button>
                </div>
              </div>
              <div class="task-content ticket-prgs-common">
                <div class="prgs-avt">
                  <img src="@/assets/img/t-image.png" /><img
                    class="prgs-avtflg"
                    src="data:image/png;base64,"
                    alt=""
                  />
                </div>
                <div class="tckt-t-w">
                  <span><b>Jobs Title</b></span>
                  <span>Project ID</span>
                </div>
                <div class="tckt-t-w">
                  <span><b>Tasks</b></span>
                  <span class="c-green"><font class="c-red">4</font> / 3</span>
                </div>
                <div class="tckt-t-w">
                  <span><b>Milestone</b></span>
                  <span class="c-green"><font class="c-red">3</font> / 2</span>
                </div>
                <div class="tckt-t-w tckt-t-w-btn">
                  <button class="btn btn-lg btn-green">Close</button>
                </div>
              </div>
              <div class="task-content ticket-prgs-common">
                <div class="prgs-avt">
                  <img src="@/assets/img/t-image.png" /><img
                    class="prgs-avtflg"
                    src="data:image/png;base64,"
                    alt=""
                  />
                </div>
                <div class="tckt-t-w">
                  <span><b>Jobs Title</b></span>
                  <span>Project ID</span>
                </div>
                <div class="tckt-t-w">
                  <span><b>Tasks</b></span>
                  <span class="c-green"><font class="c-red">4</font> / 3</span>
                </div>
                <div class="tckt-t-w">
                  <span><b>Milestone</b></span>
                  <span class="c-green"><font class="c-red">3</font> / 2</span>
                </div>
                <div class="tckt-t-w tckt-t-w-btn">
                  <button class="btn btn-lg btn-green">Close</button>
                </div>
              </div>
            </div>
          </div>
          <div class="row t-container tab-pane" id="tab-2-5">
            <div class="col-12 tab-sec-ticket tab-not">
              <div class="filter-common">
                <div class="filter-inner-comon"><span>Filter</span></div>
                <div class="filter-inner-comon">
                  <input type="text" placeholder="Search..." />
                </div>
                <div class="filter-inner-comon">
                  <select>
                    <option>Sort by</option>
                    <option>Sort 1</option>
                    <option>Sort 2</option>
                    <option>Sort 3</option>
                  </select>
                  <div class="srt-dd-active">
                    <svg
                      version="1.1"
                      id="Capa_1"
                      xmlns="http://www.w3.org/2000/svg"
                      xmlns:xlink="http://www.w3.org/1999/xlink"
                      x="0px"
                      y="0px"
                      width="255px"
                      height="255px"
                      viewBox="0 0 255 255"
                      style="enable-background: new 0 0 255 255"
                      xml:space="preserve"
                    >
                      <g>
                        <g id="arrow-drop-down">
                          <polygon
                            points="0,63.75 127.5,191.25 255,63.75   "
                          ></polygon>
                        </g>
                      </g>
                      <g></g>
                      <g></g>
                      <g></g>
                      <g></g>
                      <g></g>
                      <g></g>
                      <g></g>
                      <g></g>
                      <g></g>
                      <g></g>
                      <g></g>
                      <g></g>
                      <g></g>
                      <g></g>
                      <g></g>
                    </svg>
                  </div>
                </div>
              </div>
              <div class="tab-content">
                <div class="tab-pane show active">
                  <div class="task-content-row">
                    <div class="task-content ticket-prgs-common">
                      <div class="prgs-avt">
                        <img src="@/assets/img/t-image.png" /><img
                          class="prgs-avtflg"
                          src="data:image/png;base64,"
                          alt=""
                        />
                      </div>
                      <div class="tckt-t-w">
                        <span><b>Jobs Title</b></span>
                        <span>Project ID</span>
                      </div>
                      <div class="tckt-t-w">
                        <span><b>Tasks</b></span>
                        <span class="c-green"
                          ><font class="c-red">4</font> / 3</span
                        >
                      </div>
                      <div class="tckt-t-w">
                        <span><b>Milestone</b></span>
                        <span class="c-green"
                          ><font class="c-red">3</font> / 2</span
                        >
                      </div>
                      <div class="tckt-t-w">
                        <span><b>Status</b></span>
                        <span class="c-green">Open</span>
                      </div>
                    </div>
                    <div class="stm-abt-tckt c-red">
                      <span>Ticket ID: 1234556</span>
                      <span>Ticket Reason: Work not completed</span>
                      <span>Claimed by: Company</span>
                    </div>
                  </div>
                  <div class="task-content-row">
                    <div class="task-content ticket-prgs-common">
                      <div class="prgs-avt">
                        <img src="@/assets/img/t-image.png" /><img
                          class="prgs-avtflg"
                          src="data:image/png;base64,"
                          alt=""
                        />
                      </div>
                      <div class="tckt-t-w">
                        <span><b>Jobs Title</b></span>
                        <span>Project ID</span>
                      </div>
                      <div class="tckt-t-w">
                        <span><b>Tasks</b></span>
                        <span class="c-green"
                          ><font class="c-red">4</font> / 3</span
                        >
                      </div>
                      <div class="tckt-t-w">
                        <span><b>Milestone</b></span>
                        <span class="c-green"
                          ><font class="c-red">3</font> / 2</span
                        >
                      </div>
                      <div class="tckt-t-w">
                        <span><b>Status</b></span>
                        <span class="c-green">Open</span>
                      </div>
                    </div>
                    <div class="stm-abt-tckt c-red">
                      <span>Ticket ID: 1234556</span>
                      <span>Ticket Reason: Work not completed</span>
                      <span>Claimed by: User</span>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
          <div class="row t-container tab-pane" id="tab-2-6">
            <div class="col-12 tab-sec-ticket tab-not">
              <div class="filter-common">
                <div class="filter-inner-comon"><span>Filter</span></div>
                <div class="filter-inner-comon">
                  <input type="text" placeholder="Search..." />
                </div>
                <div class="filter-inner-comon">
                  <select>
                    <option>Sort by</option>
                    <option>Sort 1</option>
                    <option>Sort 2</option>
                    <option>Sort 3</option>
                  </select>
                  <div class="srt-dd-active">
                    <svg
                      version="1.1"
                      id="Capa_1"
                      xmlns="http://www.w3.org/2000/svg"
                      xmlns:xlink="http://www.w3.org/1999/xlink"
                      x="0px"
                      y="0px"
                      width="255px"
                      height="255px"
                      viewBox="0 0 255 255"
                      style="enable-background: new 0 0 255 255"
                      xml:space="preserve"
                    >
                      <g>
                        <g id="arrow-drop-down">
                          <polygon
                            points="0,63.75 127.5,191.25 255,63.75   "
                          ></polygon>
                        </g>
                      </g>
                      <g></g>
                      <g></g>
                      <g></g>
                      <g></g>
                      <g></g>
                      <g></g>
                      <g></g>
                      <g></g>
                      <g></g>
                      <g></g>
                      <g></g>
                      <g></g>
                      <g></g>
                      <g></g>
                      <g></g>
                    </svg>
                  </div>
                </div>
              </div>
              <div class="tab-content">
                <div class="tab-pane show active">
                  <div class="task-content-row">
                    <div class="task-content ticket-prgs-common">
                      <div class="tckt-t-w task-icon">
                        <svg class="">
                          <use xlink:href="#post-it-1"></use>
                        </svg>
                      </div>
                      <div class="task-ctnt-common task-det">
                        <span><b>Jobs Title</b></span>
                        <span
                          >Project ID : Category – SubCatgeory Date and
                          Time</span
                        >
                      </div>
                      <div class="task-ctnt-common task-det text-center">
                        <span><b>Proposals</b></span>
                        <span class="c-green">15</span>
                      </div>
                    </div>
                    <div class="stm-abt-tckt c-red">
                      <span>Reason: Job was added by Mistakes</span>
                    </div>
                  </div>
                  <div class="task-content-row">
                    <div class="task-content ticket-prgs-common">
                      <div class="tckt-t-w task-icon">
                        <svg class="">
                          <use xlink:href="#post-it-1"></use>
                        </svg>
                      </div>
                      <div class="task-ctnt-common task-det">
                        <span><b>Jobs Title</b></span>
                        <span
                          >Project ID : Category – SubCatgeory Date and
                          Time</span
                        >
                      </div>
                      <div class="task-ctnt-common task-det text-center">
                        <span><b>Proposals</b></span>
                        <span class="c-green">15</span>
                      </div>
                    </div>
                    <div class="stm-abt-tckt c-red">
                      <span>Reason: Job was added by Mistakes</span>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  </div>
</template>